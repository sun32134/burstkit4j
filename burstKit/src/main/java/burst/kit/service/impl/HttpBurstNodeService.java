...
public final class HttpBurstNodeService implements BurstNodeService {
    ...
    @Override
    public Single<Transaction[]> getTransactionsByHeight(Long height) {
        return assign(blockchainService.getTransactionsByHeight(BurstKitUtils.getEndpoint(), String.valueOf(height)))
                .map(response -> Arrays.stream(response.getTransactions())
                        .map(Transaction::new)
                        .toArray(Transaction[]::new));
    }
...
//                })
    ...
    @Override
    public Single<TransactionResponse> setRewardRecipient(BurstAddress recipient, String secretPhrase, String deadline, String feeNQT) {
        return assign(blockchainService.setRewardRecipient(BurstKitUtils.getEndpoint(), recipient.getID(), secretPhrase, deadline, feeNQT))
                .map(GenerateTransactionResponse::getTransactionResponse);
    }
    ...
    private interface BlockchainService {
        ...
        @GET("{endpoint}?requestType=getTransactionsByHeight")
        Single<AccountTransactionsResponse> getTransactionsByHeight(@Path("endpoint") String endpoint, @Query("height") String height);
        ...
        Single<GenerateTransactionResponse> setRewardRecipient(@Path("endpoint") String endpoint, @Query("recipient") String recipient, @Query("secretPhrase") String secretPhrase, @Query("deadline") String deadline, @Query("feeNQT") String feeNQT);
